...
import { ResizableEvent } from 'app/shared/resizable/resizable.directive';
...
export class ModalComponent implements OnInit, AfterViewChecked {
  ...
  @Input() text: string;
  @Input() textExist: boolean = false;
  @Input() inputStr: string = "text";
  @Input() inputExist: boolean = false;
  @Output() confirmSubmit: EventEmitter<string> = new EventEmitter();
  ...
  confirm() {
    if(this.inputExist && !this.reason) {
      this.reasonError = true;
      return;
    }
    this.cancel();
    this.confirmSubmit.emit(this.reason);
  }
